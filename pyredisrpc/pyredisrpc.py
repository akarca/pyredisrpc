...
class Server(object):
    ...
    def __init__(
        self, queue, redis_url="redis://", prefix="pyredisrpc:", response_expire_time=60
    ):
        """
    ...
    def run(self):
        ...
        while True:
            ...
            res = self.redis.blpop(self.queue, timeout=30)
            if res is None:
                self.redis = None
                continue
    ...
    def parse_request(self, req_data):
        ...
        if (
            type(params) != list
            or len(params) != 2
            or type(params[0]) != list
            or type(params[1]) != dict
        ):
